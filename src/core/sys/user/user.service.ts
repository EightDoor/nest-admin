...
export class UserService extends TypeOrmCrudService<SysUser> {
  constructor(@InjectRepository(SysUser) repo: Repository<SysUser>, @InjectRepository(SysUserRole) private readonly userRole: Repository<SysUserRole>) {
    super(repo)
  }
  ...
  async deleteOne(req: CrudRequest): Promise<SysUser> {
    ...
    // 先删除用户存在的角色
    await this.userRole.createQueryBuilder().where("user_id = :id", { id: params.id }).delete().execute()
