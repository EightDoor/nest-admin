...
export class RoleController implements CrudController<SysRole> {
  ...
  // 菜单关联
  async menuAssignment(@Body() body: SysRoleMenu, @TransactionManager() manager: EntityManager) {
    return this.menu.RoleRelationMenu(body, manager)
  }

  @Get("menus/:id")
  // 关联菜单查询列表
  async getMenus(@Param() params: {id: string}): Promise<SysRoleMenu | undefined> {
